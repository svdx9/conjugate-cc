...
- [x] #4 #3.1 The git SHA and built time are provided in the Makefile
- [x] #5 Basic backend tests or equivalent automated verification cover the initial server behavior that the MVP relies on, including the build metadata endpoint.
- [x] #6 Documentation is updated with the command(s) needed to run and verify the backend locally.
<!-- AC:END -->

## Implementation Plan

<!-- SECTION:PLAN:BEGIN -->
1. Initialize the backend Go module under `backend/` and create the first application entrypoint at `backend/cmd/main.go`, wiring the backend `Makefile` expectations (`./cmd`, `main.serviceGitSHA`, and `main.serviceBuildTime`) into a runnable binary.
...
6. Update the root developer workflow as needed for this backend bootstrap, likely by extending the existing backend `Makefile` with any additional backend support targets required by the OpenAPI/codegen flow while preserving the repo-local tooling model introduced in `TASK-001.1`.
...
8. Update `README.md` so contributors can install backend tools, run the backend locally, hit the initial verification endpoints, and understand how the generated API contract fits into the backend structure. Document the makefile commands in the README
...
Bootstrapped the Go backend service.

Key accomplishments:
- Initialized Go module in `backend/`.
- Defined OpenAPI 3.1 schema for health (`/v1/status`) and build metadata (`/v1/metadata`) endpoints.
- Generated server boilerplate using `oapi-codegen`.
- Implemented a clean internal structure following the `go-backend` skill rules:
  - `internal/config` for env-based configuration.
  - `internal/status` for feature-specific handlers.
  - `internal/http` for router wiring using `chi`.
- Implemented the composition root in `cmd/server/main.go` with support for injected `GitSHA` and `BuildTime` via `-ldflags`.
- Added graceful shutdown handling.
- Verified behavior with unit tests for status handlers.
- Updated `backend/Makefile` with `generate`, `build`, and `test` targets.
- Updated `backend/README.md` with instructions on how to install tools, generate code, and run the service.

The backend is now runnable and provides a stable base for further development.
